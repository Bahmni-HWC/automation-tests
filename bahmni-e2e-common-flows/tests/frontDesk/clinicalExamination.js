...
step("Doctor prescribe tests <prescriptions>", async function (prescriptionFile) {
    ...
    var jsonData = JSON.parse(fileExtension.parseContent(prescriptionFile))
    var labOrders = jsonData.labOrders;
    gauge.dataStore.scenarioStore.put("LabOrders", labOrders)
    for (const labOrder of labOrders) {
        gauge.message(labOrder)
        await click(labOrder.LabGroup);
        await taikoHelper.repeatUntilFound(text(labOrder.test))
        await click(labOrder.test, { force: true })
        await waitFor(100)
    }
});
    ...
    for (var i = 0; i < prescriptionsCount; i++) {
        ...
        if (medicalPrescriptions.drug_name != null) {
            ...
            if (await textBox(toRightOf("Drug Name")).exists()) {
                await write(drugName, into(textBox(toRightOf("Drug Name"),above("Dose"))));
                await click(link(drugName, below(textBox(toRightOf("Drug Name")))));
                await write(medicalPrescriptions.dose.split('-')[0], into($("#morning-dose")));
                await write(medicalPrescriptions.dose.split('-')[1], into($("#afternoon-dose")));
                await write(medicalPrescriptions.dose.split('-')[2], into($("#evening-dose")));
                await dropDown(toRightOf("Units")).select(medicalPrescriptions.units);
                await dropDown(toRightOf("Route")).select(medicalPrescriptions.route);
                await write(medicalPrescriptions.duration, into(textBox(toRightOf("Duration"))));
                await dropDown(toRightOf("Units"),below("Duration")).select(medicalPrescriptions.durationUnits);
