...
step("Verify medical prescription in patient clinical dashboard", async function () {
    ...
    for (var i = 0; i < prescriptionCount; i++) {
        ...
        // assert.ok(await text(`${medicalPrescriptions.dose} ${medicalPrescriptions.units}, ${medicalPrescriptions.route} - ${medicalPrescriptions.duration} Days`, within($("#Treatments"))).exists())
        assert.ok(await text(`${medicalPrescriptions.drug_name} (${medicalPrescriptions.units}) ${medicalPrescriptions.dose} ${medicalPrescriptions.units}, ${medicalPrescriptions.route} - ${medicalPrescriptions.duration} ${medicalPrescriptions.durationUnits}`, within($("#Treatments"))).exists())
    }
});
...
step("Verify consultation notes in patient clinical dashboard", async function () {
    ...
    await click($("#currentVisitIcon"), within($("#Visits")))
    //Work Around not sure why the Consultation Notes is not displayed on the General Tab
    await click(link("Patient Summary"))
    assert.ok(await text(consultationNote, within($("#consultationNotes-section")), toRightOf("Consultation note")).exists())
});

step("Validate the lab tests are available in patient clinical dashboard", async function () {
    var labOrders = gauge.dataStore.scenarioStore.get("LabOrders")
    for (const labOrder of labOrders) {
        assert.ok(await text(labOrder.test, within($("#Lab-Results"))).exists())
    }
});
