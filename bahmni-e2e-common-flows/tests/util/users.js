...
async function randomAddress() {
    ...
    var JsonData = jsonfile[randomRow]
    const addressJson = {};
    for (const key in JsonData) {
        const value = JsonData[key];
        const modifiedValue = value.split('%')[0];
        addressJson[key] = modifiedValue;
    }
    addressJson.buildingNumber =faker.address.buildingNumber();
    addressJson.localitySector = faker.address.secondaryAddress();
    addressJson.pinCode = faker.address.zipCode("56####");
    gauge.dataStore.scenarioStore.put("addressJson", addressJson);
    return addressJson;
}
