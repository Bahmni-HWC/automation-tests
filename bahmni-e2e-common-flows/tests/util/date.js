...
function getGetCurrentddmmyyhhmmssms() {
    const now = new Date();
    return `${now.getDate()}${now.getMonth() + 1}${String(now.getFullYear()).slice(2)}${now.getHours()}${now.getMinutes()}${now.getSeconds()}${now.getMilliseconds()}`;
}
...
function generateRandomDate(maxYears) {
    const currentDate = new Date();
    const randomYear = currentDate.getFullYear() + getRandomInt(1, maxYears);
    const randomMonth = getRandomInt(0, 11);
    const randomDay = getRandomInt(1, new Date(randomYear, randomMonth + 1, 0).getDate());

    const formattedDate = `${randomDay.toString().padStart(2, '0')}/${(randomMonth + 1).toString().padStart(2, '0')}/${randomYear}`;
...
module.exports = {
    ...
    getGetCurrentddmmyyhhmmssms: getGetCurrentddmmyyhhmmssms,
