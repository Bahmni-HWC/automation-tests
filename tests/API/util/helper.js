...
async function getLoginLocationUUID(loginLocationName) {
    return await (await httpRequests.customGet(endpoints.LOGIN_LOCATION)).body.results.filter(locations => locations.name == loginLocationName)[0].uuid;
}
...
async function startVisit(createPatientPayload) {
    ...
    //get Location UUID
    var loginLocationUUID = await getLoginLocationUUID(expectedValues.LOGIN_LOCATION_NAME);
...
async function getActivePatients() {
    return await httpRequests.customGet(endpoints.ACTIVE_PATIENT_QUEUE, { location_uuid: await getLoginLocationUUID(expectedValues.LOGIN_LOCATION_NAME), provider_uuid: await getProviderUuid() });
}
